...
odb_version="19C"
db_arr=( {bancsarc,bancsdb,bancsrep}"${node_num}" )
...
# TO BE FIXED
# <--- Database SQL Checks (Skip on Node 2) --->
if [[ "$node_num" != "2" ]]; then
    echo "Running Database SQL Health Checks..."
    # Ensure you are in the main_dir so spooled files land in the right folder
    sqlplus -s / as sysdba @/path/to/hc_queries.sql
else
    echo "Skipping SQL checks on Node 2 as per MOP."
fi
